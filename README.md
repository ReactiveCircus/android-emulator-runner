...
| `working-directory` | Optional | `./` | A custom working directory - e.g. `./android` if your root Gradle project is under the `./android` sub-directory within your repository. Will be used for `script` & `pre-emulator-launch-script` & `post-failure-script`. |
...
| `post-failure-script` | Optional | N/A | 'custom script to run after custom script failed and before killing the emulator - e.g. `./upload-tomestones.sh`' |
